...
{      
    ...
    face_image_sub_ = this->create_subscription<usr_msgs::msg::FaceDetection>("/face_detection", 
...
void FaceDetector::draw_boxes_callback_(usr_msgs::msg::FaceDetection msg)
{
    cv_bridge::CvImagePtr cv_ptr;
    cv_ptr = cv_bridge::toCvCopy(msg.image, sensor_msgs::image_encodings::RGB8);
    cv::Mat frame;
    cvtColor( cv_ptr->image, frame, cv::COLOR_BGR2RGB);
    ...
    for ( size_t i = 0; i < msg.boxes.size(); i++ )
    {
        ...
        cv::rectangle(frame, p1, p2, cv::Scalar(0,255,0), 3);
    }
    cv::imwrite("final_image.png", frame);
